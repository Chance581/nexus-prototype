...
function applyGradedSectionScores(data) {
    if (activeBenchmarkType === "general_cs") {
        ...
        sectionScores = {
            ...
            ds: Number(scores.ds) || 0,
            s: Number(scores.s) || 0,
            cc: Number(scores.cc) || 0
        };
...
async function loadPreviousScores() {
    ...
    if (!user) {
        scoresList.innerHTML =
            "<p class='subtle'>Sign in to view previous scores.</p>";
        return;
    }

    const { data: generalScores, error: generalError } = await supabaseClient
        .from("score_percentiles")
        .select("*")
        .eq("user_id", user.id);

    const { data: advancedScores, error: advancedError } = await supabaseClient
        .from("advanced_score_percentiles")
        .select("*")
        .eq("user_id", user.id);

    if (generalError || advancedError) {
        console.error("Could not load previous scores:", {
            generalError,
            advancedError
        });

        scoresList.innerHTML =
            "<p class='subtle'>Could not load previous scores.</p>";
        return;
    }
    ...
    if (data.length === 0) {
        scoresList.innerHTML =
            "<p class='subtle'>No saved scores yet.</p>";
        return;
    }
    ...
    data.forEach(scoreRow => {
        const date = new Date(scoreRow.created_at)
            .toLocaleDateString();

        const isAdvanced =
            scoreRow.benchmark_type === "advanced_cs";

        const benchmarkLabel = isAdvanced
            ? "Advanced CS"
            : "General CS";

        const categoryHTML = isAdvanced
            ? `
                <p class="subtle">
                    Advanced benchmark · Architecture, optimization,
                    abstraction, concurrency, and debugging
                </p>
            `
            : `
                <p class="subtle">
